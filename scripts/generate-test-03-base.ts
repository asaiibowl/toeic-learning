/**
 * オリジナル問題「テストセット03」のメタデータ・Part5・Part6 を生成する。
 * 公式問題の転載はせず、業務場面を題材に独自作成した問題のみを出力する。
 *
 * 実行: npx tsx scripts/generate-test-03-base.ts
 */
...
import { part5Questions } from './test-03/part5-seeds';
import { part6Passages } from './test-03/part6-seeds';

const TEST_ID = 'test-03';
...
const meta = TestMetaSchema.parse({
  testId: TEST_ID,
  title: 'テストセット03（75分・100問）',
  description:
    'TOEIC L&R Readingの現行公式構成に準拠したオリジナル100問セット（上級・860点以上帯）。Part 5: 30問、Part 6: 16問、Part 7: 単一文書29問・複数文書25問。',
  createdAt: '2026-08-07T18:00:00+09:00',
  tags: ['full-test', '75-minutes', 'original', 'expert'],
